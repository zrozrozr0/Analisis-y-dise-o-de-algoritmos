...
int main() {   
    ...
    //Assign values ??to A and B matrices, you can assign values ??arbitrarily, for testing  
    for(int i=0; i<N; i++) {   
       for(int j=0; j<N; j++) {   
           A[i][j] = i * j;   
           B[i][j] = i * j;      
    ...
    //Call Strassen method to realize C=A*B  
     Strassen(N, A, B, C);   
       
    //Output matrix C median  
     output(N, C);   
...
void Strassen(int n, T A[][N], T B[][N], T C[][N]) {   
     ...
     if(n == 2) {  //2-order  
        ...
        //Divide the matrices A and B into four sub-matrices of the same order, that is, divide and conquer.  
        for(int i=0; i<n/2; i++) {   
           for(int j=0; j<n/2; j++) {   
               A11[i][j] = A[i][j];   
               A12[i][j] = A[i][j+n/2];   
